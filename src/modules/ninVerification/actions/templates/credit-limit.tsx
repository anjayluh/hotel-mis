...
const CreditLimit = ({ action }: IProps) => {
  ...
  const fields: IRec[] = [
    {
      label: "Credit Limit",
      value: printsMoney(data.creditInformation.creditLimit),
    },
    {
      label: "Minimum",
      value: printsMoney(data.creditInformation.minimumAmount),
    },
    {
      label: "Grade",
    ...
    },
    {
      ...
      value: (
        <DataValue>
          <i>"{data.creditInformation.remarks}"</i>
        </DataValue>
      ),
    },
  ];

  const repaymentPlans = data.repaymentPlans.map((it) => ({
    ...it,
    id: getRandomStr(),
  }));
  const titles: ITitle[] = [
