...
import EditIcon from "@material-ui/icons/Edit"; // this is temporal
...
function WorkflowView({ classes, data }: IProps) {
  ...
  useEffect(() => {
    for (let doc of docs) {
      if (hasValue(documents[doc.id])) continue;
      const url = `${remoteRoutes.documentsDownload}/${doc.id}`;
      downLoad(url, (blobResp) => {
        dispatch(
          saveDocument({ id: doc.id, url: URL.createObjectURL(blobResp) })
        );
      });
    }
  }, []);
    ...
    <Stepper orientation="vertical" nonLinear={true} className={classes.root}>
      {tasks.map((task) => (
        <Step key={task.id} active={true}>
          <StepLabel StepIconComponent={EditIcon}>
