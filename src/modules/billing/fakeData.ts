...
export const fakeCase = (): IWorkflow => {
  ...
  return {
    ...
    status: faker.random.arrayElement(
      enumToArray(WorkflowStatus)
    ) as WorkflowStatus,
    subStatus: faker.random.arrayElement(
      enumToArray(WorkflowSubStatus)
    ) as WorkflowSubStatus,
...
export const organisationNames = [
...
export const fakeBill = (): IBill => {
  return {
    ...
    consumptionAmount: faker.finance.account(5),
    rate: faker.random.number({ min: 20, max: 50 }),
    lastBillAmountDue: faker.finance.account(7),
  };
};
